...
import 'package:simple_live_app/services/soop_account_service.dart';
...
class AccountController extends GetxController {
  ...
  void soopTap() async {
    if (SoopAccountService.instance.hasCookie.value) {
      var result = await Utils.showAlertDialog(
        "确定要清除 SOOP Cookie 吗？",
        title: "清除配置",
      );
      if (result) {
        SoopAccountService.instance.clearCookie();
        SmartDialog.showToast("SOOP Cookie 已清除");
      }
    } else {
      doSoopCookieConfig();
    }
  ...
  void doSoopCookieConfig() {
    var controller = TextEditingController(
      text: SoopAccountService.instance.cookie,
    );

    Get.dialog(
      AlertDialog(
        title: const Text("配置 SOOP Cookie"),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text(
                "请先在浏览器登录 SOOP，然后复制 Cookie 粘贴到这里。\n用于访问需要登录、地区或成人权限的直播间。",
                style: TextStyle(fontSize: 12, color: Colors.grey),
              ),
              const SizedBox(height: 12),
              TextField(
          ...
          TextButton(
            onPressed: () {
              ...
              if (input.isEmpty) {
                ...
                SoopAccountService.instance.setCookie(input);
                SmartDialog.showToast("SOOP Cookie 已保存");
              }
            },
            child: const Text("确定"),
          ),
